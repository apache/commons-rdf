...
class JenaGraphImpl implements JenaGraph {
	...
	public void remove(final BlankNodeOrIRI subject, final IRI predicate, final RDFTerm object) {
		graph.remove(toJenaPattern(subject), toJenaPattern(predicate), toJenaPattern(object));
	}
	...
	public void remove(final Triple triple) {
		if ((triple.getObject() instanceof Literal) && ((Literal) triple.getObject()).getLanguageTag().isPresent()) {
			...
			graph.remove(factory.asJenaNode(triple.getSubject()), factory.asJenaNode(triple.getPredicate()),
